...
function SearchShow(){
    ...
    return(
        ...
        <Link to={`/search/${encodeURI(show.name)}`} onClick={() => searchCity()} className="text-white absolute right-2.5 bottom-2.5 font-medium rounded-lg text-sm px-4 py-2 dark:bg-red-700 dark:hover:bg-red-800">Search</Link>
        </div>
        </form>
    <Outlet />
        </div>
